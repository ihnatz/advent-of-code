module Commands
  ...
  def add(modes, a, b, c)
    @memory[c] = to_arg(modes[0], a) + to_arg(modes[1], b)
    @pointer + 4
  end
  ...
  def flz(modes, a, b, c)
    @memory[c] = to_arg(modes[0], a) < to_arg(modes[1], b) ? 1 : 0
    @pointer + 4
  end
...
def check_sequence
  ...
  core.run
  raise unless core.output.last == 14522484
  raise unless core.output.slice(0..-2).all?(&:zero?)
  raise unless core.halted
  p "ok"
end
